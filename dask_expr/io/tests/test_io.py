...
def test_from_legacy_collection(optimize):
    ddf = dd.from_dict({"a": range(100)}, npartitions=10)
    df = from_legacy_collection(ddf, optimize=optimize)
    assert isinstance(df.expr, Expr)
    assert_eq(df, ddf)


@pytest.mark.parametrize("optimize", [True, False])
def test_to_legacy_collection(optimize):
    pdf = pd.DataFrame({"x": [1, 4, 3, 2, 0, 5]})
    df = from_pandas(pdf, npartitions=2)
    ddf = df.to_legacy_collection(optimize=optimize)
